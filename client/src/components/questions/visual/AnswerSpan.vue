<template>
  <div :id="id" :class="color"
    ...
    <div v-if="checked" class="w-6 h-6 flex-shrink-0">
      <img v-if="isCorrect" src="https://s.svgbox.net/hero-outline.svg?ic=check&fill=8bc34a" class="w-full" alt="correct"/>
      <img v-else src="https://s.svgbox.net/hero-outline.svg?ic=x&fill=ff5252" class="w-full" alt="incorrect"/>
    </div>
  </div>
</template>
<script>
