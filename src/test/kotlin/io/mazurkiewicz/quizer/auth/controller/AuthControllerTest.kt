package io.mazurkiewicz.quizer.auth.controller
...
import io.mazurkiewicz.quizer.auth.token.TokenService
import io.mazurkiewicz.quizer.auth.user.UserRepresentation
import io.mazurkiewicz.quizer.auth.user.UserService
import io.mazurkiewicz.quizer.exception.ApiError
import io.mazurkiewicz.quizer.exception.QuizerExceptionHandler
...
class AuthControllerTest {
    ...
    fun `should return 400 when register email is already registered`() {
        ...
        every { userService.createUser(email, password) } throws EmailAlreadyExistException(email)
    ...
    fun `should return 401 for invalid user credentials`() {
        ...
        every { userService.findUserForCredentials(email, password) } throws CredentialsNotValidException()
        every { tokenService.generateToken(any()) } returns token
